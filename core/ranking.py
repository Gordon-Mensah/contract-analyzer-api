...
from core.config import (
    FEEDBACK_FILE, MODEL_FILE, WEIGHTS_FILE,
    DEFAULT_WEIGHTS, AUTO_TRAIN_THRESHOLD,
    MIN_SAMPLES_FOR_MODEL, MIN_POSITIVE_FOR_MODEL
)
...
def score_candidate_heuristic(original: str, candidate: str) -> tuple[float, dict]:
...
def log_feedback(clause_index, original, candidate, accepted, meta, score, action):
    ...
    if len(store) >= AUTO_TRAIN_THRESHOLD:
        result = train_feedback_model(min_samples=MIN_SAMPLES_FOR_MODEL)
        if not result.get("ok"):
            print(f"⚠️ Model training skipped: {result.get('reason')}")

def load_feedback_store() -> list:
    if os.path.exists(FEEDBACK_FILE):
        with open(FEEDBACK_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return []

def save_feedback_store(store: list):
...
def train_feedback_model(min_samples=MIN_SAMPLES_FOR_MODEL) -> dict:
...
def predict_accept_prob(meta: dict) -> float | None:
